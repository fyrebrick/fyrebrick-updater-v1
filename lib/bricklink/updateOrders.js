...
module.exports = async function main(fyrebrick_id,user) {
  ...
  const progressBar = progress.create(data.length, 0,{description:"Update orders for "+user.username});
  for await (let order of data) {
    ...
    if (!db_order && order.status !== "PURGED") { //NEW
